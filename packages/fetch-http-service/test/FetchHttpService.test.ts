import { HttpService, Options, AjaxError } from 'http-service';
import { FetchHttpService } from '../src/FetchHttpService';
...
describe('should call api for data with global options', function() {
  ...
  const httpService = new HttpService(fetchService, globalOptions);
...
xdescribe('should map request before send and then eject interceptor', function() {
  afterEach(jest.clearAllMocks);

  it.todo('get method', function() {
    // @ts-ignore
    fetch.mockResponse(JSON.stringify(MOCK_RESPONSE_DATA));

    const fetchService = new FetchHttpService();
    const httpService = new HttpService(fetchService, {
      host: 'http://localhost:4000/api/',
      headers: { Authorization: 'token' },
      enabledMock: false,
      reqInterceptor: req => {
...
xdescribe('should map response before .then method and then eject interceptor', function() {
  afterEach(jest.clearAllMocks);

  it.todo('get method', async function() {
    // @ts-ignore
    fetch.mockResponse(JSON.stringify(MOCK_RESPONSE_DATA));
  ...
  it.todo('post method', async function() {
    // @ts-ignore
    fetch.mockResponse(JSON.stringify(MOCK_RESPONSE_DATA));
...
xdescribe('should map response before .then method and then eject interceptor - with mock', () => {
  afterEach(jest.clearAllMocks);
